...
extern const char *stbtt_GetFontNameString(const stbtt_fontinfo font, int platformID, int encodingID, int languageID, int nameID);
...
const char* stbtt_GetFontNameString(const stbtt_fontinfo font, int platformID, int encodingID, int languageID, int nameID)
{
   ...
   for (i=0; i < count; ++i) {
     ...
     if (platformID == ttUSHORT(fc+loc) && encodingID == ttUSHORT(fc+loc+2)
          && languageID == ttUSHORT(fc+loc+4) && nameID == ttUSHORT(fc+loc+6)) {

         char *fontTXT = (const char *) (fc+stringOffset+ttUSHORT(fc+loc+10));
